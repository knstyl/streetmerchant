import {Element, LabelQuery, Pricing} from '../store/model';
...
export async function elementIncludesLabels(
  ...
  const resolved = await Promise.all(
    elementQueries.map(async query => {
      ...
      const contents = (await extractElementContents(page, element, selector)) ?? '';
...
export async function extractElementContents(
  ...
  return page.evaluate((component, options: Selector) => {
    const element: globalThis.HTMLElement | null = component.querySelector(
      options.selector
    );
...
export async function getPrice(
  ...
  const selector = {...options, selector: query.container};
  const priceString = await extractElementContents(page, element, selector);
...
export async function getProductText(
  ...
  const productText = await extractElementContents(page, element, selector);
  return productText ?? undefined;
}
